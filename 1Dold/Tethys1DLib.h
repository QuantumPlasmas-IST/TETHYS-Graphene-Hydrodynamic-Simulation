...
class TETHYSBase {
	...
	public:
		explicit  TETHYSBase(int sizeN); // acho que pelo menos para já nao vai precisar de construtor ou entao ponho o banner mesmo no constrturos 
		~TETHYSBase();	
...
class Fluid1D : public TETHYSBase{
	...
	public :
		...
		explicit Fluid1D(int sizeN,float VELSND, float VISCO);
		~Fluid1D();
...
class GrapheneFluid1D : public Fluid1D{
	...
	public : 
		...
		explicit GrapheneFluid1D(int sizeN,float VELSND, float FERMI,float VISCO,float COL);
