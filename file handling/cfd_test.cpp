...
int main (void)
{
	...
	*/
	...
	float data_den[NX][NY];          // buffer for data to write
	float data_vel[NX][NY];          // buffer for data to write
	for (int j = 0; j < NX; j++)
	{
	  for (int i = 0; i < NY; i++){
		 data_den[j][i] = sin(i*0.1) + 2.0*sin(j*0.1);
		 data_vel[j][i] = i*0.1-j*0.1;
		}
	}
