...
Fluid2D::Fluid2D(int sizeNx, int sizeNy){		
	Nx = sizeNx;
	Ny = sizeNy;
...
float  Fluid2D::Density_Flux(float n,float velX, float velY, float S){ // Look at this (why not pure virtual function) LOOK
	float f1;
	f1 = n*v;
	return f1;		
}
float  Fluid2D::VelocityX_Flux(float n,float velX, float velY, float S){
	float f2;
	f2 = 0.5*v*v + n; 
	return f2;
}
float  Fluid2D::VelocityY_Flux(float n,float velX, float velY, float S){
	float f2;
	f2 = 0.5*v*v + n;
	return f2;
}
float  Fluid2D::DensitySource(float n,float v,float S){
	return 0;
}
float  Fluid2D::VelocityXSource(float n,float v,float S){
	return 0;
}
float  Fluid2D::VelocityYSource(float n,float v,float S){
	return 0;
}	
...
void Fluid2D::Smooth(int width){
	...
	AverageFilter( velX ,velX_cor, Nx, width);
	AverageFilter( curX ,curX_cor, Nx , width);
	AverageFilter( velY ,velY_cor, Ny, width);
	AverageFilter( curY ,curY_cor, Ny , width);
}
...
void Fluid2D::Richtmyer(){
		...
		for(int i=0; i<Nx-1; i++){
			for(int j=0; j<Ny; j++){
				...
				velX_mid_x[i][j] = 0.5*(velX_mid_x[i][j] + velX_mid_x[i+1][j]);
				velY_mid_x[i][j] = 0.5*(velY_mid_x[i][j] + velY_mid_x[i+1][j]);
			}
		}
		// mid_y
		for(int i=0; i<Nx; i++){
			for(int j=0; j<Ny-1; j++){
				...
				velX_mid_y[i][j] = 0.5*(velX_mid_y[i][j] + velX_mid_y[i][j+1]);
				velY_mid_y[i][j] = 0.5*(velY_mid_y[i][j] + velY_mid_y[i][j+1]);
			}
		}
    	//
		...
		for(int i=0; i<Nx-1; i++){
			for(int j=0; j<Ny-1; j++){
				den_mid[i][j] = 0.25*(den[i][j] + den[i+1][j] + den[i][j+1] + den[i+1][j+1])
								-0.5*dt*(DensityFlux())
			}
		}
		for ( int i = 0; i < Nx - 1; i++ )
		{
			den_mid[i] = 0.25*( den[i] + den[i+1] )
		...
		float GrapheneFluid2D::DensityFlux(float n,float v,float S){
			float f1;
			f1 = n*v;
			return f1;			
		}
		float GrapheneFluid2D::VelocityFlux(float n,float v,float S){
			float f2;
			f2 = 0.25*v*v + vel_fer*vel_fer*0.5*log(n) + 2*S*S*sqrt(n); 
			return f2;			
		}
		float GrapheneFluid2D::DensitySource(float n,float v,float S){
			float Q1=0.0;
			return Q1;				
		}
		float GrapheneFluid2D::VelocitySource(float n,float v,float S){
			float Q2=0.0;
			Q2=-1.0*col_freq*(v-1);
			return Q2;			
		}
...
void GrapheneFluid2D::CFLCondition(){
	dx = leng / ( float ) ( Nx - 1 );
