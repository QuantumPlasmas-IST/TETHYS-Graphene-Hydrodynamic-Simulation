...
#include "../../../CLionProjects/TETHYS-Graphene-Hydrodynamic-Simulation/src/includes/FluidCNP2DLibNOT.h"
...
FluidCNP2D::FluidCNP2D(const SetUpParameters &input_parameters) : TethysBase{input_parameters.SizeX, input_parameters.SizeY, 2}{
	...
	/*
	//10 total variables if we include the energy equation. 6 otherwise
	num_vars = input_parameters.NumVars;
	//8 conserved quantities if we include the energy equation. 6 otherwise
	int num_cons_vars;
	if (num_vars == 6){
		num_cons_vars = 6;
	}
...
*/
	...
	Tmp 		= new float[Nx * Ny]();
	Tmp_h		= new float[Nx * Ny]();
	
	Den 		= new float[Nx * Ny]();
	Den_h 		= new float[Nx * Ny]();
	
	VelX 		= new float[Nx * Ny]();
	VelX_h 		= new float[Nx * Ny]();

	VelY 		= new float[Nx * Ny]();
	VelY_h 		= new float[Nx * Ny]();
	
	FlxX 		= new float[Nx * Ny]();
	FlxY 		= new float[Nx * Ny]();
	
	CurX 		= new float[Nx * Ny]();
	CurX_h      = new float[Nx * Ny]();

	CurY 		= new float[Nx * Ny]();
	CurY_h      = new float[Nx * Ny]();
	...
	den_E_kin    = new float[Nx * Ny]();
	den_E_energy = new float[Nx * Ny]();
	den_E_th     = new float[Nx * Ny]();

	cur_totX = new float[Nx * Ny]();
	cur_totY = new float[Nx * Ny]();
	
	EX = new float[Nx * Ny](); //X component of the electric field
	EY = new float[Nx * Ny](); //Y component of the electric field

	//Save the values of the quantities at the interfaces
	UL_x = new float[num_vars*(Nx - 1)*(Ny - 2)]();
	UR_x = new float[num_vars*(Nx - 1)*(Ny - 2)]();

	UL_y = new float[num_vars*(Nx - 2)*(Ny - 1)]();
	UR_y = new float[num_vars*(Nx - 2)*(Ny - 1)]();

	//Compute fluxes at the interfaces along x and y
	F = new float[num_cons_vars*(Nx - 1)*(Ny - 2)]();
	G = new float[num_cons_vars*(Nx - 2)*(Ny - 1)]();

	//Electrostatic potential
	phi = new float[Nx*Ny]();

	//Save half step variables
	den_half = new float[Nx*Ny]();
	den_h_half = new float[Nx*Ny]();
	
	curX_half = new float[Nx*Ny]();
	curY_half = new float[Nx*Ny]();
	
	curX_h_half = new float[Nx*Ny]();
	curY_h_half = new float[Nx*Ny]();

	e_half = new float[Nx*Ny]();
	e_h_half = new float[Nx*Ny]();

	p_half = new float[Nx*Ny]();
	p_h_half = new float[Nx*Ny]();

	t_half = new float[Nx*Ny]();
	t_h_half = new float[Nx*Ny]();
...
FluidCNP2D::~FluidCNP2D() = default;

void FluidCNP2D::SetSound(){
...
void FluidCNP2D::InitialCondRand(){
	random_device rd;
	float maxrand;
...
void FluidCNP2D::InitialCondTest(){
	for (int i = 0; i < Nx; i++ ){
		for (int j=0; j<Ny; j++){
			float densi;
...
void FluidCNP2D::MassFluxToVelocity(){
...
void FluidCNP2D::VelocityToCurrent() {
	for(int c=0; c <= Nx * Ny - 1; c++){
		CurX[c] = VelX[c] * Den[c];
		CurY[c] = VelY[c] * Den[c];
	}
...
void FluidCNP2D::WriteFluidFile(float t){
	...
	for (int i = 0; i < Nx*Ny; ++i){
		if(Den[i] < 0.f || Den_h[i] < 0.f || !isfinite(VelX[i]) || !isfinite(VelX_h[i])){
...
void FluidCNP2D::SetSimulationTime(){
	Tmax=5.0f+0.02f*vel_snd+20.0f/vel_snd;
}

void FluidCNP2D::VelocityLaplacianFtcs() {
...
void FluidCNP2D::VelocityLaplacianWeighted19() {
	this->MassFluxToVelocity();
...
void FluidCNP2D::TemperatureLaplacianWeighted19() {
...
void FluidCNP2D:: ParabolicOperatorFtcs() {
	VelocityLaplacianFtcs();
	ForwardTimeOperator();
}

void FluidCNP2D::ParabolicOperatorWeightedExplicit19() {
...
void FluidCNP2D::ReadSnapShot(const H5std_string &snap_name) {
...
void FluidCNP2D::SaveSnapShot() {
	...
	DataSet dataset_den_h = GrpDen_h->createDataSet(name_dataset, HDF5FLOAT, *DataspaceDen_h);
	Attribute atr_step_den_h = dataset_den_h.createAttribute("time step", HDF5INT, atr_dataspace);
	Attribute atr_time_den_h = dataset_den_h.createAttribute("time", HDF5FLOAT, atr_dataspace);
    dataset_den_h.write(Den_h, HDF5FLOAT);
    dataset_den_h.close();
	atr_step_den_h.write(HDF5INT, &TimeStepCounter);
	atr_time_den_h.write(HDF5FLOAT , &currenttime);
	atr_step_den_h.close();
	atr_time_den_h.close();

	DataSet dataset_EX = GrpEX->createDataSet(name_dataset, HDF5FLOAT, *DataspaceEX);
	Attribute atr_step_EX = dataset_EX.createAttribute("time step", HDF5INT, atr_dataspace);
	Attribute atr_time_EX = dataset_EX.createAttribute("time", HDF5FLOAT, atr_dataspace);
    dataset_EX.write(EX, HDF5FLOAT);
    dataset_EX.close();
	atr_step_EX.write(HDF5INT, &TimeStepCounter);
	atr_time_EX.write(HDF5FLOAT , &currenttime);
	atr_step_EX.close();
	atr_time_EX.close();

	DataSet dataset_EY = GrpEY->createDataSet(name_dataset, HDF5FLOAT, *DataspaceEY);
	Attribute atr_step_EY = dataset_EY.createAttribute("time step", HDF5INT, atr_dataspace);
	Attribute atr_time_EY = dataset_EY.createAttribute("time", HDF5FLOAT, atr_dataspace);
    dataset_EY.write(EY, HDF5FLOAT);
    dataset_EY.close();
	atr_step_EY.write(HDF5INT, &TimeStepCounter);
	atr_time_EY.write(HDF5FLOAT , &currenttime);
	atr_step_EY.close();
	atr_time_EY.close();
	...
	DataSet dataset_vel_x_h = GrpVelX_h->createDataSet(name_dataset, HDF5FLOAT, *DataspaceVelX_h);
	Attribute atr_step_vel_x_h = dataset_vel_x_h.createAttribute("time step", HDF5INT, atr_dataspace);
	Attribute atr_time_vel_x_h = dataset_vel_x_h.createAttribute("time", HDF5FLOAT, atr_dataspace);
	dataset_vel_x_h.write(VelX_h, HDF5FLOAT);
	dataset_vel_x_h.close();
	atr_step_vel_x_h.write(HDF5INT, &TimeStepCounter);
	atr_time_vel_x_h.write(HDF5FLOAT , &currenttime);
	atr_step_vel_x_h.close();
	atr_time_vel_x_h.close();

	DataSet dataset_vel_y_h = GrpVelY_h->createDataSet(name_dataset, HDF5FLOAT, *DataspaceVelY_h);
	Attribute atr_step_vel_y_h = dataset_vel_y_h.createAttribute("time step", HDF5INT, atr_dataspace);
	Attribute atr_time_vel_y_h = dataset_vel_y_h.createAttribute("time", HDF5FLOAT, atr_dataspace);
	dataset_vel_y_h.write(VelY_h, HDF5FLOAT);
	dataset_vel_y_h.close();
	atr_step_vel_y_h.write(HDF5INT, &TimeStepCounter);
	atr_time_vel_y_h.write(HDF5FLOAT , &currenttime);
	atr_step_vel_y_h.close();
	atr_time_vel_y_h.close();
...
int FluidCNP2D::GetSnapshotStep() const { return snapshot_step;}
int FluidCNP2D::GetSnapshotFreq() const {return snapshot_per_period;}

bool FluidCNP2D::Snapshot() const {
	bool state;
...
float FluidCNP2D::DensitySource(__attribute__((unused)) float n, __attribute__((unused)) float flx_x, __attribute__((unused)) float flx_y, __attribute__((unused)) float mass, __attribute__((unused)) float s) {
	return 0.0f;
}

float FluidCNP2D::XMomentumSource(__attribute__((unused)) float n, __attribute__((unused)) float flx_x, __attribute__((unused)) float flx_y, __attribute__((unused)) float mass, __attribute__((unused)) float s) {
	return 0.0f;
}
float FluidCNP2D::YMomentumSource(__attribute__((unused)) float n, __attribute__((unused)) float flx_x, __attribute__((unused)) float flx_y, __attribute__((unused)) float mass, __attribute__((unused)) float s) {
	return 0.0f;
}

void FluidCNP2D::ForwardTimeOperator() {
...
void FluidCNP2D::VelocityGradient() {
	int stride = Nx;
...
void FluidCNP2D::VelocityGradientMid() {
	float m_east,m_west,m_north,m_south;
...
void FluidCNP2D::DensityGradient() {
	int stride = Nx;
...
void FluidCNP2D::DensityGradientMid() {
...
float FluidCNP2D::DensityToMass(float density) {
	return density;
}

float FluidCNP2D::DensityFluxX(GridPoint p, char side) {
	float * px_ptr;
	float px =0.0f;
	if(p.IsMidGrid){
		px_ptr = FlxX;
...
float FluidCNP2D::DensityFluxY(GridPoint p, char side) {
	float * py_ptr;
	float py=0.0f;
	if(p.IsMidGrid){
		py_ptr = FlxY;
...
float FluidCNP2D::XMomentumFluxX(GridPoint p, char side) {
	float * den_ptr;
	float * px_ptr;
	float den=1.0f;
	float px=0.0f;
...
float FluidCNP2D::XMomentumFluxY(GridPoint p, char side) {
	float * den_ptr;
	float * px_ptr;
	float * py_ptr;
	float den =1.0f;
...
float FluidCNP2D::YMomentumFluxY(GridPoint p, char side) {
	float * den_ptr;
	float * py_ptr;
	float den=1.0f;
	float py =0.0f;
...
float FluidCNP2D::YMomentumFluxX(GridPoint p, char side) {
	float * den_ptr;
	float * px_ptr;
	float * py_ptr;
	float den=1.0f;
...
float FluidCNP2D::TemperatureFluxX(GridPoint p, char side) {
    float * px_ptr;
	float * den_ptr;
	float * tmp_ptr;
	float tmp=1.0f;
...
float FluidCNP2D::TemperatureFluxY(GridPoint p, char side) {
    float * py_ptr;
    float py=0.0f;
	float * tmp_ptr;
	float tmp=1.0f;
...
float FluidCNP2D::Laplacian19(GridPoint p, float *input_ptr, float constant) {
...
float FluidCNP2D::TemperatureSource(float n, float flx_x, float flx_y, float den_grad_x, float den_grad_y, float mass, float s) {
	return 0;
}
